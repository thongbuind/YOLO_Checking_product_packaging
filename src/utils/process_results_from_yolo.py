...
def process_single_camera(yolo_results: dict, frame: np.ndarray, cam_info: CamInfo, cam_lock: threading.Lock):
    ...
    with cam_lock:
        if len(slots_boxes) != 5: # khi chạy thật bằng camera thì để if len(slots_boxes) == 0 cũng được, để cover cho những frame yolo detect thiếu, thì vẫn còn data cũ
        ...
        if len(slots_boxes) == 5:
            ...
            offset = 0
            if 1 not in cam_info.slots_list and 6 in cam_info.slots_list:
                offset = 5
            for slot_id, original_idx in slot_mapping.items():
                adjusted_slot_id = slot_id + offset
                slot = cam_info.get_slot(adjusted_slot_id)
                if slot is not None:
                    slot.update(slots_boxes[original_idx])
            ...
            for item_name, item_points in items_boxes:
                for slot_id in cam_info.slot_will_be_checked:
                    ...
                    if is_item_in_slot(item_points, slot_points):
        ...
        elif had_one_slot_empty: # nếu có 1 slot empy (và ko slot nào wrong đã lọc ở trên)
            cam_info.set_state("checking")
                
    draw_visualization(frame, cam_info, items_boxes)

def process_results_from_yolo(frames: dict, yolo_results: dict, cameras: dict, camera_locks: dict, executor: ThreadPoolExecutor):
    futures = []
